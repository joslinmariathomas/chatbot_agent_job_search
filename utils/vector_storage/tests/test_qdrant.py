...
def test_create_collection():
    ...
    qdrant_storage = QdrantStorage(client_server="http://localhost:6333")
...
def test_retrieve_docs_based_on_query():
    ...
    retrieved_points = qdrant_storage.retrieve_docs_based_on_query(
        query=query, limit=limit, collection_name=collection_name
    )
    assert len(retrieved_points) == limit
    payload_keys = ["name", "year", "author", "description"]
